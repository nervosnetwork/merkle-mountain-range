...
fn test_mmr(count: u32, proof_elem: Vec<u32>) {
    ...
    let result = proof.verify(root).unwrap();
    assert!(result);
}
...
//     let proof = mmr.gen_postfix_proof(vec![pos]).expect("gen proof");
//     let new_elem = count;
...
#[test]
fn test_gen_postfix_proof_with_duplicate_leaves() {
    test_mmr(10, vec![5, 5]);
}
...
fn test_invalid_proof_verification(
    ...
    match mmr.gen_postfix_proof(positions_to_verify.clone()) {
        Ok(proof) => {
            assert!(proof.verify(root.clone()).unwrap());
